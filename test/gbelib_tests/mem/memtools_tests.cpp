...
TEST(MemoryToolsTest, testTranslateAdressSafe4) {
  ...
  EXPECT_ANY_THROW(
      mem_tools::translateAdressSafe(inputAddress, startAddress, size));
}
...
TEST(MemoryToolsTest, testTranslateAdressSafe5) {
  ...
  address_type result;
  ASSERT_NO_THROW(result = mem_tools::translateAdressSafe(inputAddress,
                                                          startAddress, size));
...
TEST(MemoryToolsTest, testTranslateAdressSafe8) {
  ...
  address_type result;
  ASSERT_NO_THROW(
      result = mem_tools::translateAdressSafe(inputAddress, area, offset));

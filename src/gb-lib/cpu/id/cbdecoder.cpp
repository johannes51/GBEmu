...
auto CbDecoder::decode(const Location<uint8_t>& opcodeLocation) -> OperationUP
{
  ...
  if (opcode.value() == 0xCB) {
    return std::make_unique<CbPrefix>(this);
  } else if (indirect(opcode)) {
...
auto CbDecoder::function(const OpcodeView& opcode) -> CbFunction
{
  if (opcode.upperNibble() >= 0xC) {
    return CbFunction::Set;
  } else if (opcode.upperNibble() >= 0x9) {
...
auto CbDecoder::indirect(const OpcodeView& opcode) -> bool
{
  return opcode.lowerNibble() == 0x6 || opcode.lowerNibble() == 0xE;
}

auto CbDecoder::operand(const OpcodeView& opcode) -> ByteRegister
{
...
auto CbDecoder::bit(const OpcodeView& opcode) -> unsigned
{
